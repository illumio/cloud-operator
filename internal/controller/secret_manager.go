...
func (sm *SecretManager) ReadCredentialsK8sSecrets(ctx context.Context, secretName string) (string, string, error) {
	...
	if err != nil {
		sm.Logger.Errorw("Failed to get secret", "error", err)
		return "", "", err
	}
	...
	if clientID == "" {
		sm.Logger.Errorw("Cannot get client_id", "error", err)
		return "", "", errors.New("failed to get client_id from secret")
	}
	clientSecret := string(secret.Data["client_secret"])
	if clientSecret == "" {
		sm.Logger.Errorw("Cannot get client_secret", "error", err)
...
func (sm *SecretManager) WriteK8sSecret(ctx context.Context, keyData OnboardResponse, ClusterCreds string) error {
	...
	if err != nil {
		sm.Logger.Errorw("Failed to update secret", "error", err)
		return err
	}
	return nil
}
