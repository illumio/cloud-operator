...
func (rm *ResourceManager) sendResourceSnapshotComplete() error {
	if err := rm.streamManager.instance.stream.Send(&pb.SendKubernetesResourcesRequest{Request: &pb.SendKubernetesResourcesRequest_ResourceSnapshotComplete{}}); err != nil {
		rm.logger.Error("Falied to send resource snapshot complete", "error", err)
		return err
	}
	return nil
}
...
func (rm *ResourceManager) sendClusterMetadata(ctx context.Context) error {
	...
	if err != nil {
		rm.logger.Error("Error getting cluster id", "error", err)
	}
	clientset, err := NewClientSet()
	if err != nil {
		rm.logger.Error("Error creating clientset", "error", err)
	}
	kubernetesVersion, err := clientset.Discovery().ServerVersion()
	if err != nil {
		rm.logger.Error("Error getting Kubernetes version", "error", err)
	}
	if err := rm.streamManager.instance.stream.Send(&pb.SendKubernetesResourcesRequest{Request: &pb.SendKubernetesResourcesRequest_ClusterMetadata{ClusterMetadata: &pb.KubernetesClusterMetadata{Uid: clusterUid, KubernetesVersion: kubernetesVersion.String(), OperatorVersion: version.Version()}}}); err != nil {
		rm.logger.Error("Failed to send cluster metadata", "error", err)
		return err
	}
	return nil
}
...
func (r *ResourceManager) DyanmicListAndWatchResources(ctx context.Context, cancel context.CancelFunc, resource string, allResourcesSnapshotted *sync.WaitGroup, snapshotCompleted *sync.WaitGroup) {
	...
	if err != nil {
		...
		r.logger.Error("Unable to list resources", "error", err)
		cancel()
		return
	}
	allResourcesSnapshotted.Done()
	...
	if err != nil {
		r.logger.Error("Unable to watch events", "error", err)
		cancel()
		return
	}
}
...
func (r *ResourceManager) DynamicListResources(ctx context.Context, resource string) (string, CacheManager, error) {
	...
	for _, obj := range objs {
		...
		if err != nil {
			r.logger.Error("Cannot send object metadata", "error", err)
			return "", cm, err
		}
		hashValue, err := hashObjectMeta(obj)
		if err != nil {
			r.logger.Error("Cannot hash current object", "error", err)
			return "", cm, err
		}
...
func (r *ResourceManager) watchEvents(ctx context.Context, resource string, watchOptions metav1.ListOptions, c CacheManager) error {
	...
	if err != nil {
		r.logger.Error("Error setting up watch on resource", "error", err)
		return err
	}
	for event := range watcher.ResultChan() {
		switch event.Type {
		case watch.Error:
			r.logger.Error("Watcher event has returned an error", "error", err)
			return err
		case watch.Bookmark:
			continue
		default:
		...
		if err != nil {
			r.logger.Error("Cannot convert runtime.Object to metav1.ObjectMeta", "error", err)
			return err
		}
		...
		if err != nil {
			r.logger.Error("Failed to hash object metadata", "error", err)
			return err
		}
		...
		if err != nil {
			r.logger.Error("Cannot send resource mutation", "error", err)
			return err
		}
...
func (r *ResourceManager) listResources(ctx context.Context, resource schema.GroupVersionResource, namespace string) ([]metav1.ObjectMeta, string, error) {
	...
	if err != nil {
		r.logger.Error("Cannot list resource", "error", err, "kind", resource)
		return nil, "", err
	}
	...
	for _, item := range unstructuredResources.Items {
		...
		if err != nil {
			r.logger.Error("Cannot get Metadata from resource", "error", err)
			return nil, "", err
		}
