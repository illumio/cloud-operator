...
// Egress to kube-dns: empty endpointSelector, toEndpoints kube-dns, toPorts UDP 53.
func TestProtoToMap_KubeDNSEgressRule(t *testing.T) {
	udpProto := "UDP"
	...
	result, err := protoToMap(rule)
	require.NoError(t, err)

	// camelCase keys, not snake_case
	assert.Contains(t, result, "endpointSelector")
	assert.NotContains(t, result, "endpoint_selector")

	// empty endpointSelector
	es, ok := result["endpointSelector"].(map[string]any)
	require.True(t, ok)
	assert.Empty(t, es)

	egress, ok := result["egress"].([]any)
	require.True(t, ok)
	require.Len(t, egress, 1)
	egressRule, ok := egress[0].(map[string]any)
	require.True(t, ok)

	// toEndpoints with kube-dns labels
	...
	// toPorts UDP 53
	toPorts, ok := egressRule["toPorts"].([]any)
	require.True(t, ok)
...
func TestProtoToMap_CiliumCIDRGroup(t *testing.T) {
	cidrGroup := &pb.CiliumCIDRGroup{
		ExternalCidrs: []string{"10.0.0.0/8", "192.168.0.0/16"},
	}

	result, err := protoToMap(cidrGroup)
	require.NoError(t, err)

	cidrs, ok := result["externalCidrs"].([]any)
	require.True(t, ok)
	assert.Equal(t, []any{"10.0.0.0/8", "192.168.0.0/16"}, cidrs)
	assert.NotContains(t, result, "external_cidrs", "should use camelCase")
}

func TestProtoToMap_OmitsUnsetFields(t *testing.T) {
	rule := &pb.CiliumPolicyRule{
		EndpointSelector: &pb.LabelSelector{
	...
	assert.NotContains(t, result, "nodeSelector")
	assert.NotContains(t, result, "enableDefaultDeny")
	assert.NotContains(t, result, "ingress")
	assert.NotContains(t, result, "egress")
	assert.NotContains(t, result, "ingressDeny")
	assert.NotContains(t, result, "egressDeny")
