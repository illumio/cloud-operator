...
// IncrementPolicyMutations increments the count of policy mutations.
func (s *Stats) IncrementPolicyMutations() {
	s.policyMutations.Add(1)
}

// GetAndResetStats returns the current stats and resets all counters to zero.
func (s *Stats) GetAndResetStats() (flowsReceived, flowsSent, resourceMutations, policyMutations uint64) {
	...
	policyMutations = s.policyMutations.Swap(0)
		...
		for {
			...
			case <-ticker.C:
				...
				logger.Info("Stream statistics",
					...
					zap.Uint64("policy_mutations", policyMutations),
				)
			}
		}
	}()
